...
const EmptyState = (
    ...
    <div
        className="panel-strong mx-auto flex h-16 w-16 items-center justify-center rounded-[1.4rem] bg-[hsl(var(--surface-1))]">
        <Icon className="h-8 w-8 text-muted-foreground" />
    </div>
    <h3 className="font-display mt-4 text-base font-semibold text-foreground">{title}</h3>
    <p
        className="mx-auto mt-2 max-w-[280px] text-sm leading-relaxed text-muted-foreground">{description}</p>
...
const Index = () => {
    ...
    useEffect(() => {
        ...
        if (!requestId || tab !== "bids" || bidRequests.length === 0 || role !== "engineer")
            return;
    ...
    const handleToggleCompanyVisibility = async () => {
        ...
        try {
            ...
            showSuccess(
                nextVisibility ? "Tu empresa ahora es visible para clientes." : "Tu empresa ahora está oculta para clientes."
            );
        } catch {
    ...
    const renderBidRequestsBlock = () => {
        ...
        return (
            <div className="space-y-4">
                ...
                {bidRequestsQuery.isLoading ? (<div className="space-y-3">
                    {Array.from({
                        length: 3
                    }).map((_, index) => (<div key={index} className="app-shell space-y-4 p-5">
                    ...
                    action={<div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
                        <Button
                            onClick={() => bidRequestsQuery.refetch()}
                            variant="outline"
                            className="gap-2">
                            <RefreshCw className="h-4 w-4" />Reintentar
                                            </Button>
                        <Button onClick={() => setIsCreateModalOpen(true)}>Crear solicitud</Button>
                    </div>} />) : engineerRequests.length === 0 ? (<EmptyState
                    icon={ClipboardList}
                    ...
                    action={<Button onClick={() => setIsCreateModalOpen(true)}>
                        <Plus className="h-4 w-4" />Crear solicitud
                                      </Button>} />) : (<div className="space-y-3">
                    {engineerRequests.map(
                        request => (<article key={request.id} className="app-shell interactive-card p-5">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                                <span className="data-chip data-chip-accent bg-[#f5a10f]">{request.category}</span>
                                <span
                                    className={cn("data-chip", request.status === "completed" ? "" : "data-chip-success")}>
                            ...
                            <div className="panel-muted my-4 p-4">
                                ...
                                {request.items.length > 0 ? (<ul className="mt-3 space-y-2">
                                    {request.items.map(item => (<li
                                        key={item.id ?? item.name}
                                        className="flex items-center justify-between gap-3 border-b border-border/70 pb-2 last:border-b-0 last:pb-0">
                            ...
                            <div
                                className="mt-4 flex items-end justify-between gap-4 border-t border-border/80 pt-4">
                                <div>
    ...
    const renderMarketBlock = () => {
                    ...
                    <section className="app-shell p-5">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                ...
                                <p className="mt-1 text-sm leading-relaxed text-muted-foreground">Administra cómo te ven los compradores dentro del mercado con datos reales y configuración controlada.
                                                    </p>
                            </div>
                    ...
                    <section className="app-shell p-5">
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                <span
                                    className={cn("toggle-thumb", profile?.is_public ? "translate-x-7" : "translate-x-0")} />
                            </button>
                        </div>
                        <div className="panel-muted mt-5 p-4">
                            <p className="section-label">Perfil público</p>
                            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">Completa tu nombre comercial, sector y cobertura para mejorar tu presencia en el marketplace.
                                              </p>
                            <Button
        ...
        if (marketplaceStoresQuery.isLoading) {
            return (
                <div className="space-y-3">{Array.from({
                        length: 3
                    }).map(
                        (_, index) => <div key={index} className="app-shell h-40 animate-pulse p-5" />
                    )}</div>
            );
        }
        ...
        if (marketplaceStoresQuery.error) {
            return (
                <EmptyState
                    ...
                    action={<Button onClick={() => marketplaceStoresQuery.refetch()} className="gap-2">
                        <RefreshCw className="h-4 w-4" />Reintentar
                                    </Button>} />
            );
        }
        ...
        if (marketplaceStores.length === 0) {
            return (
                <EmptyState
                    ...
                    action={<Button variant="outline" onClick={() => setActiveTab("bids")}>Volver a subastas
                                    </Button>} />
            );
        }
        ...
        return (
            <section className="app-shell p-5">
                <div className="flex items-center gap-3 border-b border-border/80 pb-4">
                    <div
                        className="flex h-14 w-14 items-center justify-center rounded-[1.25rem] bg-[hsl(var(--primary)/0.14)] font-display text-lg font-semibold text-[hsl(var(--warning-foreground))]">
                        {initials}
                    </div>
                    <div>
                ...
                {isIncomplete ? (<div className="panel-muted mt-4 p-4">
                    <p className="font-display text-sm font-semibold text-foreground">Perfil incompleto</p>
                    <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Completa tu información para evitar espacios vacíos en tu experiencia productiva.
                                    </p>
                    {role === "hardware" ? (<Button
                        variant="outline"
                        onClick={() => setIsProfileModalOpen(true)}
                        className="mt-4 w-full">Completar perfil de empresa
                                      </Button>) : null}
                </div>) : null}
                {profile?.user_type === "hardware" && (<div className="panel-muted mt-4 flex items-center justify-between p-4">
                ...
                <div className="mt-4 space-y-2">
                    {profile?.user_type === "hardware" && (<button
                        onClick={() => setIsProfileModalOpen(true)}
                    ...
                    {role === "hardware" && (<button
                        ...
                        className="interactive-row flex min-h-[48px] w-full items-center rounded-[1.1rem] px-4 py-3 text-left text-sm text-foreground hover:bg-[hsl(var(--surface-2))]">Mi perfil de empresa
                                    </button>)}
                    <button
                        onClick={signOut}
                        className="interactive-row flex min-h-[48px] w-full items-center rounded-[1.1rem] px-4 py-3 text-left text-sm text-destructive hover:bg-destructive/10">Cerrar sesión
                                  </button>
                </div>
            </section>
        );
    ...
    return (
        <div className="min-h-screen bg-[hsl(var(--surface-0))] px-0 md:px-6 md:py-6">
            <div
                className="mx-auto flex min-h-screen w-full max-w-md flex-col border-x border-border bg-background md:min-h-[calc(100vh-3rem)] md:overflow-hidden md:rounded-[2rem] md:border md:shadow-[0_28px_60px_-40px_hsl(var(--foreground)/0.45)]">
                <Header />
