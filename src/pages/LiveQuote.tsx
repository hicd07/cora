                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-4 space-y-4">
                        ...
                        <div className="panel-muted p-5">
                            ...
                            {isLoadingConversations ? (<div className="space-y-3">
                                <Skeleton className="h-12 w-full" />
                                <Skeleton className="h-12 w-full" />
                            </div>) : conversations.length === 0 ? (<p className="text-xs text-muted-foreground text-center py-4">No hay conversaciones activas.</p>) : (<div className="space-y-3">
                                {conversations.map(conv => (<div
                                    ...
                                    onClick={() => setActiveConversation({
                                        id: conv.id,
                                        storeName: conv.external_stores?.name || conv.wa_phone_number
                                    })}>
                                    <div className="overflow-hidden">
                                        ...
                                        <p
                                            className={`text-xs mt-0.5 flex items-center gap-1 ${conv.state === "ACTIVE" ? "text-destructive font-medium" : "text-muted-foreground"}`}>
                                            <MessageCircle className="h-3 w-3" />
                                            {conv.state === "ACTIVE" ? "Requiere Atención (Humano)" : conv.state}
                                        </p>
                                    </div>
                                    <Button variant="ghost" size="icon" className="shrink-0 h-8 w-8">
                                        {conv.state === "ACTIVE" && <div
                                            className="h-2 w-2 rounded-full bg-destructive absolute top-2 right-2 animate-pulse" />}
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between border-b pb-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h2 className="font-display font-bold text-base text-foreground">Ofertas vía WhatsApp (IA)
                                                                  </h2>
                                    </div>
                                    <Badge
                                        variant="secondary"
                                ...
                                {aiBids.length === 0 ? (<div className="panel-muted p-6 text-center text-sm text-muted-foreground">Aún no se han recibido ofertas estructuradas por WhatsApp.
                                                        </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {aiBids.map(bid => {
                                        const total = bid.offers.reduce(
                                            (acc, offer) => acc + offer.unitPrice * (request.items.find(i => i.name === offer.itemName)?.quantity || 1),
                                        ...
                                        return (
                                            <div
                                                key={bid.id}
                                                className="panel-strong p-5 border-purple-500/10 bg-purple-500/[0.01] animate-in fade-in zoom-in-95 duration-300">
                                                <div className="flex justify-between items-start mb-3">
                                                    <div className="flex items-center gap-2">
                                                        <div
                                                    ...
                                                    <Badge
                                                        variant="secondary"
                                                        className="bg-purple-500/10 text-purple-600 border-purple-500/20">WhatsApp IA
                                                                                      </Badge>
                                                </div>
                                                <div className="space-y-2 mt-4">
                                                    {bid.offers.slice(0, 3).map((offer, idx) => (<div key={idx} className="flex justify-between text-sm">
