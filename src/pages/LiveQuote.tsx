...
export default function LiveQuote() {
  ...
  const handleSeedAiBids = async () => {
    if (!request) return;
    setIsSeeding(true);
    const toastId = showLoading("Generando 3 ofertas de WhatsApp...");

    try {
      const mockStores = [
        { name: "Ferretería SDE Express (WhatsApp)", delivery: "Inmediato (1-2 horas)" },
        { name: "Materiales El Primo (WhatsApp)", delivery: "Mismo día (4-6 horas)" },
        { name: "Ferretería El Progreso (WhatsApp)", delivery: "Siguiente día (24 horas)" }
      ];

      for (const store of mockStores) {
        // 1. Insert hardware_bid
        const { data: bid, error: bidError } = await supabase
          .from("hardware_bids")
          .insert({
            request_id: request.id,
            store_name: store.name,
            delivery_time: store.delivery,
        ...
        // 2. Insert bid_offers for each item
        const offersToInsert = request.items.map((item) => {
          const basePrice = item.name.toLowerCase().includes("cemento") ? 480 
                          : item.name.toLowerCase().includes("varilla") ? 380
                          : item.name.toLowerCase().includes("bloque") ? 50
                          : 1500;
          const randomFactor = 0.9 + Math.random() * 0.2; // +/- 10%
          const unitPrice = Math.round(basePrice * randomFactor);
      ...
      showSuccess("Se han generado 3 ofertas de WhatsApp con éxito.");
      queryClient.invalidateQueries({ queryKey: ["request-bids", request.id] });
    } catch (error: any) {
      dismissToast(toastId);
      showError(error.message || "Error al generar ofertas.");
    } finally {
      setIsSeeding(false);
    }
  };
          ...
          <div className="lg:col-span-8 space-y-8">
            {isLoadingBids ? (
              ...
              <div className="panel-muted p-8 text-center flex flex-col items-center justify-center">
                ...
                <Button 
                  onClick={handleSeedAiBids} 
                  disabled={isSeeding}
                  className="mt-4 gap-2"
                >
                  <Sparkles className="h-4 w-4" />
                  Simular 3 ofertas de WhatsApp (IA)
                </Button>
              </div>
            ) : (
              <>
                  ...
                  <div className="flex items-center justify-between border-b pb-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button 
                        ...
                        variant="outline"
                        size="sm"
                        className="h-7 text-xs gap-1 border-purple-500/30 hover:bg-purple-500/10 text-purple-600 dark:text-purple-400"
                      >
