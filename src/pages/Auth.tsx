...
const onboardingSteps = [{
    ...
    description: "Entiende en segundos cómo PIDO ordena compras, subastas y seguimiento comercial."
}, {
    title: "Entra o crea tu cuenta",
    description: "Puedes saltar directo al login o comenzar el registro desde esta misma pantalla."
}, {
    title: "Completa tu perfil",
    description: "Activa tu experiencia como ingeniero o ferretería y empieza a operar con contexto real."
}] as const;
...
export const Auth: React.FC = () => {
    ...
    const authPanelCopy = useMemo(() => {
        ...
        if (isSignUp) {
            return {
                eyebrow: "Paso 2 · Crear cuenta",
                title: "Activa tu acceso",
                description: "Crea tu cuenta para continuar al onboarding y configurar tu operación."
            };
        }

        return {
            eyebrow: "Paso 2 · Iniciar sesión",
            title: "Entra a tu panel",
            description: "Si ya conoces PIDO, puedes saltarte la bienvenida y entrar directamente."
        };
    ...
    return (
        <div
            ...
            <div
                className="mx-auto flex min-h-[calc(100vh-2.5rem)] w-full max-w-6xl flex-col gap-4 md:min-h-[calc(100vh-4rem)]">
                ...
                <div className="grid flex-1 gap-4 md:grid-cols-[1.08fr_0.92fr]">
                    <section className="app-shell relative overflow-hidden p-6 md:p-8">
                        <div
                            className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,hsl(var(--primary)/0.2),transparent_34%),radial-gradient(circle_at_bottom_right,hsl(var(--primary)/0.12),transparent_34%)]" />
                        <div
                            className="panel-strong absolute inset-x-6 top-6 h-32 rounded-[2rem] border-primary/15 bg-[hsl(var(--primary)/0.08)] blur-2xl md:inset-x-8" />
                        <div
                            className="absolute right-6 top-8 hidden h-24 w-24 rounded-full border border-[hsl(var(--primary)/0.18)] bg-[hsl(var(--surface-1)/0.55)] blur-[2px] md:block motion-safe:animate-[float_7s_ease-in-out_infinite]" />
                        <div
                            className="absolute bottom-12 left-8 hidden h-16 w-16 rounded-full border border-[hsl(var(--foreground)/0.08)] bg-[hsl(var(--surface-1)/0.7)] md:block motion-safe:animate-[float_9s_ease-in-out_infinite]" />
                        <div className="relative z-10 flex h-full flex-col justify-between gap-8">
                            <div className="motion-safe:animate-[fade-up_500ms_ease-out]">
                                <div className="flex flex-wrap items-center gap-3">
                                    <div
                                        className="relative flex h-14 w-14 items-center justify-center overflow-hidden rounded-[1.4rem] border border-[hsl(var(--foreground)/0.08)] bg-[hsl(var(--surface-1))] shadow-[0_18px_32px_-24px_hsl(var(--foreground)/0.3)]">
                                        <div
                                            className="absolute inset-[4px] rounded-[1.05rem] bg-[hsl(var(--primary)/0.08)]" />
                                ...
                                <h2
                                    ...
                                    {isOnboarding ? "Último paso: define tu perfil y activa tu operación." : "Te damos la bienvenida a una experiencia de compras B2B más clara."}
                                </h2>
                                <p
                            ...
                            <div
                                className="grid gap-3 md:grid-cols-3 motion-safe:animate-[fade-up_700ms_ease-out]">
                                {onboardingSteps.map((step, index) => (<article
                                    key={step.title}
                                    className="panel-muted flex h-full flex-col gap-3 p-4 motion-safe:animate-[fade-up_700ms_ease-out]"
                                    style={{
                                        animationDelay: `${index * 90}ms`
                                    }}>
                                    <div className="flex items-center justify-between gap-3">
                                        <span className="data-chip">Paso {index + 1}</span>
                    ...
                    <section className="app-shell overflow-hidden md:self-center">
                        <div
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div>
                                    <p className="section-label">{authPanelCopy.eyebrow}</p>
                                    <h2 className="font-display mt-2 text-2xl font-semibold text-foreground">{authPanelCopy.title}</h2>
                                    <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground">{authPanelCopy.description}</p>
                                </div>
                                <div
                            ...
                            {!isOnboarding ? (<form onSubmit={handleAuth} className="space-y-5">
                                <div className="panel-muted p-4">
                                    ...
                                    <p className="mt-2 text-sm leading-relaxed text-foreground">
                                        {isSignUp ? "Crea tu cuenta y te llevamos al siguiente paso para completar tu perfil profesional o comercial." : "Entra con tu cuenta si ya conoces la plataforma y quieres continuar sin pasos adicionales."}
                                    </p>
                                </div>
                                ...
                                <div className="text-center">
                                    <button
                                        ...
                                        {isSignUp ? "¿Ya tienes una cuenta? Inicia sesión" : "¿Primera vez aquí? Crear cuenta"}
                                    </button>
                                </div>
