...
const slides = [
  {
    ...
    mockup: (
      <div className="panel-muted w-full p-4 interactive-card relative overflow-hidden">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-semibold text-muted-foreground">Cotización #128</span>
          <span className="data-chip data-chip-accent">4 ofertas</span>
        </div>
        <div className="space-y-2">
          {[
            { price: "$12,450", provider: "Ferretería El Sol", best: true },
            { price: "$13,100", provider: "Materiales del Este", best: false },
            { price: "$13,850", provider: "Constructor SR", best: false },
          ].map((offer, i) => (
            <div key={i} className="app-shell flex items-center justify-between p-3 interactive-row">
  ...
  },
  {
    ...
    mockup: (
      <div className="app-shell flex flex-col items-center justify-center p-6 interactive-card relative overflow-hidden">
        <div className="absolute top-0 h-16 w-full bg-primary/10"></div>
        <div className="relative mt-4 flex h-20 w-20 items-center justify-center rounded-2xl bg-surface-2 border-2 border-primary/20 text-3xl font-bold text-primary shadow-sm">
          F
          <div className="absolute -bottom-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-primary text-primary-foreground border-2 border-surface-1">
  ...
  },
  {
    ...
    mockup: (
      <div className="panel-strong w-full p-5 interactive-card">
        <div className="mb-4 flex items-center gap-2">
          <MapPin className="h-4 w-4 text-primary" />
          <span className="text-sm font-semibold text-foreground">Cobertura activa</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {["SDE", "Distrito Nacional", "SDN", "Boca Chica"].map((zone, i) => (
            <div key={i} className="app-shell flex flex-col items-center justify-center py-3 text-center">
        ...
        <div className="mt-4 flex items-center justify-between rounded-xl bg-surface-2 p-3 border border-border">
...
export const WelcomeCarousel: React.FC<WelcomeCarouselProps> = ({
  ...
  return (
    <div className="flex min-h-[100dvh] w-full flex-col bg-background relative overflow-hidden">
      ...
      <div className="absolute top-0 -left-10 w-72 h-72 bg-primary/10 rounded-full blur-3xl opacity-50 pointer-events-none" />
      <div className="absolute bottom-40 -right-10 w-96 h-96 bg-accent/30 rounded-full blur-3xl opacity-50 pointer-events-none" />
      
      {/* Top bar */}
      <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between p-4 md:px-8 animate-fade-down" style={{ animationDelay: '100ms' }}>
        <AppLogo />
        <button
          onClick={toggleTheme}
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col items-center justify-center w-full max-w-2xl mx-auto pt-16 pb-32">
        <Carousel
          setApi={setApi}
          opts={{ loop: false }}
          className="w-full"
      ...
      {/* Footer / Controls */}
      <div className="fixed bottom-0 left-0 right-0 z-50 flex flex-col items-center bg-gradient-to-t from-background via-background to-transparent pb-8 pt-12 px-6">
        <div className="w-full max-w-md mx-auto space-y-8">
          ...
          <div className="flex flex-col gap-3 animate-fade-up" style={{ animationDelay: '550ms' }}>
            <Button 
              size="lg" 
