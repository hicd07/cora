...
import { Bell, BellOff, Clock3, Gavel, Package, Warehouse, CheckCircle2, Edit3 } from "lucide-react";
...
interface OpportunityFeedProps {
  requests: (BidRequest & { userBidId?: string | null })[];
      ...
      {!isAvailable ? (
        ...
        <section className="panel-muted rounded-[1.8rem] border-dashed p-8 text-center">
          ...
          <p className="mx-auto mt-1 max-w-[260px] text-xs leading-relaxed text-muted-foreground">
            Aún no hay solicitudes activas en tu zona. Cuando aparezcan, las verás aquí.
          </p>
        </section>
      ) : (
        <section className="space-y-3">
          ...
          {activeRequests.map((request) => {
            const hasBid = !!request.userBidId;
            return (
              <article key={request.id} className={cn(
                "app-shell interactive-card p-5 relative overflow-hidden transition-all",
                hasBid && "border-primary/20 bg-primary/[0.02]"
              )}>
                {hasBid && (
                  <div className="absolute right-0 top-0 rounded-bl-2xl bg-primary px-3 py-1 text-[10px] font-bold text-white flex items-center gap-1">
                ...
                <Button 
                  onClick={() => onOpenBidModal(request)} 
                  className={cn("mt-4 w-full justify-center gap-2", hasBid ? "bg-accent hover:bg-accent/90 text-accent-foreground" : "bg-primary")}
                >
                  {hasBid ? (
                    <>
